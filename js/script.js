...
let p = parameter
 ...
 for (i=0;i<p.length;i++) {
    ...
    const names = p[i][Object.keys(p[i])[0]].map(item =>`<option data-d="${item.description}">${item.name}</option>`).join(" ");
 ...
 $(document).on("change","select.param", function() {
    ...
    // Retrieve the custom data attribute
    let desc = selectedOption.data("d");

    // Store the value in a variable
    let f = desc;

    $(this).parents("tr").find(".desc").html(f)
...
$("#level-select").on("change", function(){
    ...
    p = parameter
    for (i=0;i<p.length;i++) {
        p[i][Object.keys(p[i])[0]] =  p[i][Object.keys(p[i])[0]].filter(item => item.level < selectedOption)
